...
export function EmprestimoStatusCell({
  ...
  const badge = (
    <span className="inline-flex max-w-[110px] shrink-0 items-center justify-center overflow-hidden text-ellipsis whitespace-nowrap">
      <EmprestimoStatusBadge status={status} />
    </span>
  );

  if (!isEmAberto) {
    return <span className="inline-flex shrink-0 p-1">{badge}</span>;
  }
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            type="button"
            variant="ghost"
            className="h-8 max-w-[140px] shrink-0 rounded-md px-2 py-0 hover:bg-slate-100"
        ...
        >
          ...
          <DropdownMenuItem
            ...
            className="flex cursor-pointer items-start gap-2 rounded-lg px-2 py-2 text-sm focus:bg-slate-100"
          >
